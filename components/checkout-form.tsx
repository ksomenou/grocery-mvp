...
import { deliveryFeeForSubtotal, formatLineItem, formatMoney, freeDeliveryThresholdCents } from "@/lib/format"
...
export function CheckoutForm({
  ...
  const total = Math.max(0, subtotal - discountCents) + activeDeliveryFee
  ...
  const formIsValid = !validationErrors.customerName &&
    ...
    <div className="two-col checkout-layout">
      <form className="panel form-grid checkout-form-panel" onSubmit={submit}>
        ...
        {appliedDiscountCode ? (
          ...
          <button className="button secondary checkout-discount-action" disabled={discountLoading || !discountCode.trim()} onClick={applyDiscount} type="button">
        ...
        <button className="button checkout-pay-button" disabled={loading || invalidItems.length > 0 || !formIsValid} type="submit">
          {loading ? "Redirecting to secure checkout..." : "🔒 Pay with Stripe"}
        </button>
        <div className="payment-trust" aria-label="Payment security and accepted payment methods">
          <div>
            <span><span aria-hidden="true">🔒</span> Secure checkout</span>
            <span><span aria-hidden="true">⚡</span> Fast order processing</span>
            <span><span aria-hidden="true">💳</span> Stripe protected</span>
          </div>
          <p>No subscriptions • One-time purchase</p>
        </div>
      </form>
        ...
        <div className="checkout-summary-details">
          ...
          {scheduleLabel ? (
            <div className="summary-line delivery-eta">
              ...
              <strong><span aria-hidden="true" />{scheduleLabel}</strong>
            </div>
          ) : null}
          {fulfillmentMethod === "DELIVERY" ? (
